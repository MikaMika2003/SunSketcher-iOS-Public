...
struct LearnMoreScreen1: View {
    @Environment(\.dismiss) private var dismiss
    let prefs = UserDefaults.standard
    
    let txt = "A collaborative, crowd-sourced experiment to collect observations during the 2024 solar eclipse that will determine the exact shape of the Sun."
        ...
        GeometryReader { geo in
            ...
            .onAppear {
                if prefs.bool(forKey: "Learn checked") {
                    prefs.removeObject(forKey: "Learn checked")
                    dismiss()
                }
            }
